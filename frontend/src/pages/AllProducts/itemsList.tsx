...
const ItemsList = ({ currentItems, type, column }: any) => {
  ...
  return (
    <Grid container className="banner-content w-100" columns={{ xs: 10, sm: 12, md: 12, lg: 12 }} justifyContent={`${type === "category" ? "start" : "start"}`}>
      {type === "category" && currentItems && currentItems.map((item: any, i: any) => {
        return <Grid size={{ xs:10, sm: 4, md: 2, lg: 3 }} key={i} className="mx-5 mx-md-0 mb-3 mb-lg-0">
            {type === "category" && (
              <div className="allCateProductSec" onClick={() => handleClick(item.path, "category")}>
                <Img src={item.catImg} alt="cateImage" className="sizeh200px" />
                <H3 smFt>{item.catName}</H3>
      ...
      {type === "product" && currentItems && currentItems.map((item: any, i: any) => {
        return <Grid size={{ xs:10, sm: column, md: column, lg: column }} key={i} className="mb-3 mb-lg-0">
            {type === "product" && (
